...
That package can read files from osu folder called osu!.db, collection.db and scores.db

About struct that db look on peppy site: [osu! database format docs](https://osu.ppy.sh/help/wiki/osu!_File_Formats/Db_%28file_format%29)
...
## Example (how to read scores.db)

```js
const fs = require("fs");
const { OsuDBParser } = require("osu-db-parser");

let scoresBuffer = Buffer.from(fs.readFileSync("<PATH-TO SCORES-DB>"));
const scoresDB = new OsuDBParser(null, null, scoresBuffer);

let osuScoresData = scoresDB.getScoresData();
```
